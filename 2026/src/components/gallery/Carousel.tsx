...
const Carousel: React.FC<CarouselProps> = ({
  ...
  const { user } = useAuth();
  const viewedSlides = useRef<Set<number>>(new Set([externalIndex ?? 3]));
  ...
  // Track viewed slides
  useEffect(() => {
    if (user?.pid && !user?.completedTasks?.some(t => t.taskKey === 'd9m3s8v2')) {
      viewedSlides.current.add(currentIndex);
      if (viewedSlides.current.size >= videoData.length && !hasTriggeredCompletion.current) {
        hasTriggeredCompletion.current = true;
        void completeTask('d9m3s8v2');
      }
    }
  }, [currentIndex, user, completeTask]);
