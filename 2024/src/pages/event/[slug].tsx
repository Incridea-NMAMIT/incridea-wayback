...
import { archiveEvents, getArchiveEvent } from "@/src/archive/events";
...
export async function getStaticProps({ params }: { params: Params }) {
  const event = getArchiveEvent(params.slug.split("-").pop());
  return {
    props: {
      event,
      error: event ? null : "Could not find event",
    },
  };
}

export async function getStaticPaths() {
  const paths = archiveEvents.map((event) => ({
    params: {
